# Agent Rules for Apache Pekko Persistence R2DBC
...
- Changes must be tested against at least one supported database (PostgreSQL, YugabyteDB).
...
- R2DBC integration tests require a running PostgreSQL or YugabyteDB instance. If unavailable, record in `Tests`.
...
- Reactive streams backpressure must be respected in all database operations.
